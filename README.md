...
- Replies with built-in status output or Codex SDK-backed output
...
- A working local Codex login/session for Codex-backed replies
...
- the bridge uses the Codex SDK and lets Codex manage its local/session auth
- the bridge does not include a direct OpenAI Responses API reply backend
- the `status` command reports the effective Codex SDK source and auth mode so quota failures are easier to interpret
...
- `ping`: returns `pong`
- `status`: returns bridge status and Codex SDK configuration details
...
The bridge uses `@openai/codex-sdk` and streams structured thread events.
Codex authentication is handled by the local Codex runtime/session rather than by a direct API client in this bridge.
